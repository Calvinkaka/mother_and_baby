...
class RegisterView(View):
    ...
    def post(self, request):
        ...
        # 发送激活邮件，包含激活连接(包含用户身份信息并且加密):http://127.0.0.1:8000/user/active/1
        # 加密用户的身份信息，生成激活token
        ...
        # receiver = [email]
        # html_message = '<h1>%s，欢迎您成为母婴社区会员</h1>请点击下面链接激活您的账户<br/>' \
...
class UserInfoView(LoginRequiredMixin, View):
    ...
    def get(self, request):
        ...
        # 获取用户的最近浏览记录
        # sr = StrictRedis(host='192.168.218.130', port='6379', db=9)
...
class UserOrderView(LoginRequiredMixin, View):
    ...
    def get(self, request, page):
        ...
        # 获取用户的订单信息
        user = request.user
        # todo orders = OrderInfo.objects.filter(user=user).order_by('-create_time')
